...
////////////////////////////////////////////////////////////////////////////////////
// BEGIN ARDAIS changes to defaults for configuration variables
////////////////////////////////////////////////////////////////////////////////////
...
function overlib() {
  ...
  //ARDAIS FIX: The positioning code can get confused when the div
  // is inside something unusual, such as an absolutely/relatively
  // positioned element.  So here, we make sure that the dev is a
  // direct child of the body before we do anything else.
  //
	if (ie4) {
		var theDiv = o3_frame.document.all["overDiv"];
		if (o3_frame.document.body != theDiv.parentElement) {
		  o3_frame.document.body.appendChild(theDiv);
		}
	}
...
function placeLayer() {
	...
	if (o3_fixy > -1) {
		...
		//ARDAIS FIX: Moved next two lines up from the following "if" so that iheight is always defined
		if (ie4) iheight = eval('o3_frame.'+docRoot+'.clientHeight');
		if (ns4 || ns6) iheight = o3_frame.innerHeight;

		// If VAUTO, decide what to use.
		if (o3_vauto == 1) {
			if ( (o3_y - scrolloffset) > ((eval(iheight)) / 2)) {
				o3_vpos = ABOVE;
			} else {
				o3_vpos = BELOW;
			}
		...
		// From mouse
		//ARDAIS FIX: Moved the following "if" from inside the o3_vpos == ABOVE "if" so that
		//o3_aboveheight is always defined
    	if (o3_aboveheight == 0) {
		...
		//ARDAIS FIX: Added following block to prevent tooltip from scrolling off bottom if possible
		//This change includes moving the placeY < scrolloffset test out of the above if to here so
		//that it is always executed.
		var overflowY = (placeY - scrolloffset + o3_aboveheight) - iheight;
		if (overflowY > 0) placeY = placeY - overflowY;
...
//ARDAIS FIX: In next line, use clientX/Y instead of X/Y
	if (ie5) {o3_x=eval('event.clientX+o3_frame.'+docRoot+'.scrollLeft'); o3_y=eval('event.clientY+o3_frame.'+docRoot+'.scrollTop');}
