...
if(organ == null) organ = "";
%>
 <select name="TissueType" >
 <option value="Any" selected>Any</option>
 <%for (int i=0; i<values.size(); i++){ 
 %>

        <% GbossValue value = (GbossValue) values.get(i); %>
        <option value="<%=value.getCui()%>"><%=value.getDescription()%></option>
                 
        <% }%>
</select>
        
<input type='hidden' name="compName" value="Final_Pathology_Diagnosis">
<input type="Button" name="Open" value="..." onClick="window.open('<html:rewrite page="/orm/Dispatch?op=GetDxTcHierarchy&type=T"/>','Diagpage','width=200,height=350,top=160,left=230,scrollbars=yes');">