<%@ page language="java" errorPage="/hiddenJsps/reportError/errorReport.jsp"%>
<%@ page import="com.ardais.bigr.orm.helpers.BigrGbossData" %>
<%@ page import="com.ardais.bigr.util.ArtsConstants" %>
<%@ page import="com.gulfstreambio.gboss.GbossValue" %>
<%@ page import="com.gulfstreambio.gboss.GbossValueSet" %>
<%@ page import="java.util.List" %>
<%@ taglib uri="/tld/struts-html" prefix="html" %>
<%
	com.ardais.bigr.orm.helpers.FormLogic.commonPageActions(request, response, this.getServletContext(),"P");
%>
<%
GbossValueSet valueSet = BigrGbossData.getValueSetAlphabetized(ArtsConstants.VALUE_SET_DIAGNOSIS_HIERARCHY);
List values = valueSet.getValues();
%>

 <select name="DiseaseType" onPropertyChange="showOther1()">
  <% 
  String original = (String)request.getAttribute("original");
  String diagnosis_concept_id = (String)request.getAttribute("diagnosis_concept_id");
  if(original != null && original.equals("Not available")) { %>
  <option value="">Select</option>
  <% } else { %>
  <option value="<%= diagnosis_concept_id %>"><%= original %></option>
  <% } %>
 <%for (int i=0; i<values.size(); i++){ 
 %>

        <% GbossValue value = (GbossValue) values.get(i); %>
        <option value="<%=value.getCui().trim()%>"><%=value.getDescription()%></option>
                 
        <% }%>
</select>
        
<input type='hidden' name="compName1" value="DiseaseType">
<input type="Button" name="Open" value="..." onClick="window.open('<html:rewrite page="/orm/Dispatch?op=GetLIMSDxTcHierarchy&type=D"/>','Diagpage','width=700,height=350,top=160,left=230,scrollbars=yes');">
