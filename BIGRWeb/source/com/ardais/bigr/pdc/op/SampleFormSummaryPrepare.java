...
import com.ardais.bigr.iltds.beans.SampleHome;
 ...
 * @author: Eric Zhang
  **/
public class SampleFormSummaryPrepare extends StandardOperation {
...
/**
 * SampleFormSummaryPrepare constructor comment.
 * @param req javax.servlet.http.HttpServletRequest
 * @param res javax.servlet.http.HttpServletResponse
 * @param ctx javax.servlet.ServletContext
 */
public SampleFormSummaryPrepare(javax.servlet.http.HttpServletRequest req, javax.servlet.http.HttpServletResponse res, javax.servlet.ServletContext ctx) {
	super(req, res, ctx);
}
/**
 */
public void invoke() {
    ...
    SampleHome spHome = (SampleHome) EjbHomes.getHome(SampleHome.class);
    Sample sample  = spHome.findByPrimaryKey(new SampleKey(sampleId));
    SampleOperationHome smOpHome = (SampleOperationHome) EjbHomes.getHome(SampleOperationHome.class);
    SampleOperation smOperation = smOpHome.create();
    ...
    if (sample != null) {
     ...
     /* if (!isReadOnly) {
      ...
      if(attachFileName != null && attachFileBody != null) {
        ...
        attach.setArdaisAccountKey(sample.getArdais_acct_key());
        attach.setSampleBarCodeId(sampleId);
        ...
        smOperation.insertSampleAttachment(attach, securityInfo);
  ...
  //  request.setAttribute("readOnly", "true");
    ...
    request.setAttribute("sample", "Yes");
       
    forward("/hiddenJsps/dataImport/attachmentSuccess.jsp?sampleData.sampleId="+sampleId);
    ...
    for (Iterator itr = fileItems.iterator(); itr.hasNext();  ) {
      ...
      if (item.isFormField()) {
       // System.out.println("I am in item name="+item.getFieldName() + " ="+item.getString());
          if(item.getFieldName().equals("sampleData.sampleId"))
            sampleId = JspHelper.safeTrim(item.getString());
