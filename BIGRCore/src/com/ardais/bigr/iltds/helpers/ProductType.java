...
/**
 * These are the valid BIGR product types.  These represent the values that
 * are stored in the ITEM_TYPE column in ILTDS_REQUEST_ITEM and ILTDS_LOGICAL_REPOS_ITEM
 * tables (and possibly other places).
 * 
...
public final class ProductType implements Serializable {
  ...
  public final static ProductType DNA = getOrCreateInstance("DNA");
  public final static ProductType PROTEIN = getOrCreateInstance("PROTEIN");
  public final static ProductType RNA = getOrCreateInstance("RNA");
  public final static ProductType SAMPLE = getOrCreateInstance("SAMPLE");
   ...
   * @return The type instance.
   */
  public static ProductType getInstance(String type, boolean exceptionIfNotFound) {
    synchronized (_types) {
      ProductType result = (ProductType) _types.get(type);
   ...
   * @return The type instance.
   */
  private static ProductType getOrCreateInstance(String type) {
    synchronized (_types) {
      ProductType result = (ProductType) _types.get(type);
      if (result == null) {
        result = new ProductType(type);
