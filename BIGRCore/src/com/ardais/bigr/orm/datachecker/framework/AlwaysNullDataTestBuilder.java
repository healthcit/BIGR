...
/**
 * Build a data test case for the &lt;AlwaysNull&gt; XML tag.
 */
public class AlwaysNullDataTestBuilder extends DataTestBuilderBase {
/**
 ...
 */
public AlwaysNullDataTestBuilder(DataTestBuilderFactory factory) {
	super(factory);
}
...
public Test buildTest(Node domNode, DataTestBuilderContext context) {
	...
	// An AlwaysNull tag expands into one test case for each column name listed
	// within it.  So we need to create a suite to contain these tests.
	//
	TestSuite suite = new TestSuite(id + " AlwaysNull");
	...
	//
	{
		...
		while (iter.hasNext()) {
			...
			Test test = new AlwaysNullTestCase(id + " " + columnName + " AlwaysNull",
