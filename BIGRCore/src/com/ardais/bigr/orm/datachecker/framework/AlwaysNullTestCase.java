...
/**
 * This test type corresponds to the <code>&lt;AlwaysNull&gt;</code>
 * XML tag in an XML data tests description.
 */
public class AlwaysNullTestCase extends DataRulesTestCase {
	private String _tableName = null;
	private String _columnName = null;
	private String _when = null;
/**
 * Create an AlwaysNullTestCase.
 *
 ...
 */
  public AlwaysNullTestCase(String name, String tableName, String columnName, String when) {
	super(name);
...
/**
 * Run an AlwaysNull test case.  This verifies that the specified column in
 * the specified table is  null.
 */
public void runTest() throws Exception {
	// Build the query.  The general structure of the query we build is:
	//
	// select count(1) from <tableName> where not <columnName> is null [and (<when>)]
	//
	// A specific example of such a query is::
	//
	// select count(1) from ILTDS_INFORMED_CONSENT where not CONSENT_ID is null

	StringBuffer queryBuffer = new StringBuffer(1024);

	queryBuffer.append("select count(1) from " + _tableName + "\nwhere " + _columnName + " is not null\n");
