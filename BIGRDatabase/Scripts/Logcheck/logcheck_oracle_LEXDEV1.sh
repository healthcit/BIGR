...
#SYSADMIN=root
SYSADMIN=dba_info@healthcit.com
...
$LOGTAIL /u01/app/oracle/admin/LEXDEV1/bdump/alert_LEXDEV1.log >> $TMPDIR/check.$$
...
if [ -f "$VIOLATIONS_FILE" ]; then
	if $GREP  -f $VIOLATIONS_FILE $TMPDIR/check.$$ |
	   $GREP -v -f $VIOLATIONS_IGNORE_FILE > $TMPDIR/checkoutput.$$; then
		echo >> $TMPDIR/checkreport.$$
		echo "Oracle Errors in LEXDEV1" >> $TMPDIR/checkreport.$$
		echo "=-=-=-=-=-=-=-=-=-=" >> $TMPDIR/checkreport.$$
		cat $TMPDIR/checkoutput.$$ >> $TMPDIR/checkreport.$$
		FOUND=1
	fi
