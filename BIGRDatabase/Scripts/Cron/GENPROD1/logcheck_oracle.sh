...
TMPDIR=/home/app/oracle/scripts/maint/tmp
...
HACKING_FILE=/home/app/oracle/scripts/maint/logcheck.hacking
...
VIOLATIONS_FILE=/home/app/oracle/scripts/maint/logcheck.violations
...
VIOLATIONS_IGNORE_FILE=/home/app/oracle/scripts/maint/logcheck.violations.ignore
...
IGNORE_FILE=/home/app/oracle/scripts/maint/logcheck.ignore
...
$LOGTAIL /home/app/oracle/admin/GENPROD1/bdump/alert_GENPROD1.log >> $TMPDIR/check.$$
...
if [ -f "$HACKING_FILE" ]; then
	if $GREP  -f $HACKING_FILE $TMPDIR/check.$$ > $TMPDIR/checkoutput.$$; then
...
if [ -f "$VIOLATIONS_FILE" ]; then
	if $GREP  -f $VIOLATIONS_FILE $TMPDIR/check.$$ |
	   $GREP -v -f $VIOLATIONS_IGNORE_FILE > $TMPDIR/checkoutput.$$; then
		echo >> $TMPDIR/checkreport.$$
		echo "Oracle Errors in GENPROD1" >> $TMPDIR/checkreport.$$
		echo "=-=-=-=-=-=-=-=-=-=" >> $TMPDIR/checkreport.$$
		cat $TMPDIR/checkoutput.$$ >> $TMPDIR/checkreport.$$
		FOUND=1
	fi
