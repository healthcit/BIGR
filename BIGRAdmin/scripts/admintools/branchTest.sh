#!SHELLMARKER
#
# Create a CVS branch for BIGR testing/development, for example DET testing.
# It prompts for:
#
#   - A build name, e.g. RILEY
#   - A build number, e.g. 5_2_0_52
#   - A branch date, in the format YYYYMMDD, e.g. 20070509
#   - Additional text to place in the branch tag name to describe the purpose
#     of the branch.  This must contain only letters and digits, for example
#     no spaces are allowed.  The script doesn't currently validate this text.
#
# It creates the branch against the pre-existing build tag for the specified
# build name/number, and creates a branch tag name in a standard format
...
echo "Please enter the build name: e.g. TASHA"
# No default.
d=""
echo -n "[${d}]: "
read BUILD_NAME
if [ "X${BUILD_NAME}" = "X" ] ; then
  BUILD_NAME=${d}
fi
echo

if [ "X${BUILD_NAME}" = "X" ] ; then
  die "You must specify a build name. Please re-run." 1
fi
...
echo "Please enter the descriptive tag for the branch purpose,"
echo "letters/digits only, no spaces, e.g. DET12"
# No default.
d=""
echo -n "[${d}]: "
read BRANCH_PURPOSE
if [ "X${BRANCH_PURPOSE}" = "X" ] ; then
  BRANCH_PURPOSE=${d}
fi
echo

if [ "X${BRANCH_PURPOSE}" = "X" ] ; then
  die "You must specify a branch purpose. Please re-run." 1
fi

BRANCH_AGAINST_TAG="${BUILD_NAME}_${BUILD_NUM}"
BRANCH_TAG_NAME="TEST_${BRANCH_DATE}_${BUILD_NAME}_${BUILD_NUM}_${BRANCH_PURPOSE}"
